...
Demo:         Snowflake Evaluation Program - Customer Requirements Demo
Version:      v2.0
...
Customer Data Quality Requirements:
  • JobNumber = VARCHAR + generic prefix mandatory
...
Run: DESCRIBE TABLE SNOWFLAKE_EVAL.DATA_ENGINEERING.TA_APPLICATION_DATA_BRONZE;
----------------------------------------------------------------------------------*/
...
ALTER TABLE SNOWFLAKE_EVAL.DATA_ENGINEERING.TA_APPLICATION_DATA_BRONZE
...
CREATE OR REPLACE DATA METRIC FUNCTION SNOWFLAKE_EVAL.DATA_ENGINEERING.INVALID_JOBNUMBER_FORMAT_COUNT(
...
'SELECT COUNT_IF(
    IN_COL IS NOT NULL 
    AND FALSE = (IN_COL ILIKE ''EVAL%'')
) FROM IN_TABLE';

-- Grant access to the custom function
GRANT ALL ON FUNCTION SNOWFLAKE_EVAL.DATA_ENGINEERING.INVALID_JOBNUMBER_FORMAT_COUNT(TABLE(STRING)) TO ROLE PUBLIC;

-- Test the custom function
SELECT SNOWFLAKE_EVAL.DATA_ENGINEERING.INVALID_JOBNUMBER_FORMAT_COUNT(
    SELECT JOBNUMBER FROM SNOWFLAKE_EVAL.DATA_ENGINEERING.TA_APPLICATION_DATA_BRONZE
) AS INVALID_JOBNUMBER_FORMAT_COUNT;

-- Add the custom DMF to the table for JobNumber validation
ALTER TABLE SNOWFLAKE_EVAL.DATA_ENGINEERING.TA_APPLICATION_DATA_BRONZE 
    ADD DATA METRIC FUNCTION SNOWFLAKE_EVAL.DATA_ENGINEERING.INVALID_JOBNUMBER_FORMAT_COUNT ON (JOBNUMBER);

-- Keep the existing WorkdayID validation as it's also a customer requirement
CREATE OR REPLACE DATA METRIC FUNCTION SNOWFLAKE_EVAL.DATA_ENGINEERING.INVALID_WORKDAYID_FORMAT_COUNT(
    IN_TABLE TABLE(IN_COL STRING)
)
RETURNS NUMBER 
AS
...
-- Grant access to the custom function
GRANT ALL ON FUNCTION SNOWFLAKE_EVAL.DATA_ENGINEERING.INVALID_WORKDAYID_FORMAT_COUNT(TABLE(STRING)) TO ROLE PUBLIC;

-- Add the WorkdayID validation DMF
ALTER TABLE SNOWFLAKE_EVAL.DATA_ENGINEERING.TA_APPLICATION_DATA_BRONZE 
    ADD DATA METRIC FUNCTION SNOWFLAKE_EVAL.DATA_ENGINEERING.INVALID_WORKDAYID_FORMAT_COUNT ON (WORKDAYID);

-- Create custom DMF for JobStatus validation (Customer Requirement)
CREATE OR REPLACE DATA METRIC FUNCTION SNOWFLAKE_EVAL.DATA_ENGINEERING.INVALID_JOBSTATUS_COUNT(
    IN_TABLE TABLE(IN_COL STRING)
)
RETURNS NUMBER 
AS
...
-- Grant access to the custom function
GRANT ALL ON FUNCTION SNOWFLAKE_EVAL.DATA_ENGINEERING.INVALID_JOBSTATUS_COUNT(TABLE(STRING)) TO ROLE PUBLIC;

-- Test the custom function
SELECT SNOWFLAKE_EVAL.DATA_ENGINEERING.INVALID_JOBSTATUS_COUNT(
    SELECT JOBSTATUS FROM SNOWFLAKE_EVAL.DATA_ENGINEERING.TA_APPLICATION_DATA_BRONZE
) AS INVALID_JOBSTATUS_COUNT;

-- Add the JobStatus validation DMF
ALTER TABLE SNOWFLAKE_EVAL.DATA_ENGINEERING.TA_APPLICATION_DATA_BRONZE 
    ADD DATA METRIC FUNCTION SNOWFLAKE_EVAL.DATA_ENGINEERING.INVALID_JOBSTATUS_COUNT ON (JOBSTATUS);

-- Create custom DMF for CandidateStatus validation (Customer Requirement)
CREATE OR REPLACE DATA METRIC FUNCTION SNOWFLAKE_EVAL.DATA_ENGINEERING.INVALID_CANDIDATESTATUS_COUNT(
    IN_TABLE TABLE(IN_COL STRING)
)
RETURNS NUMBER 
AS
...
-- Grant access to the custom function
GRANT ALL ON FUNCTION SNOWFLAKE_EVAL.DATA_ENGINEERING.INVALID_CANDIDATESTATUS_COUNT(TABLE(STRING)) TO ROLE PUBLIC;

-- Test the custom function
SELECT SNOWFLAKE_EVAL.DATA_ENGINEERING.INVALID_CANDIDATESTATUS_COUNT(
    SELECT CANDIDATESTATUS FROM SNOWFLAKE_EVAL.DATA_ENGINEERING.TA_APPLICATION_DATA_BRONZE
) AS INVALID_CANDIDATESTATUS_COUNT;

-- Add the CandidateStatus validation DMF
ALTER TABLE SNOWFLAKE_EVAL.DATA_ENGINEERING.TA_APPLICATION_DATA_BRONZE 
    ADD DATA METRIC FUNCTION SNOWFLAKE_EVAL.DATA_ENGINEERING.INVALID_CANDIDATESTATUS_COUNT ON (CANDIDATESTATUS);
...
SHOW PARAMETERS LIKE 'DATA_METRIC_SCHEDULE' IN TABLE SNOWFLAKE_EVAL.DATA_ENGINEERING.TA_APPLICATION_DATA_BRONZE;
...
FROM TABLE(INFORMATION_SCHEMA.DATA_METRIC_FUNCTION_REFERENCES(
    ref_entity_name => 'SNOWFLAKE_EVAL.DATA_ENGINEERING.TA_APPLICATION_DATA_BRONZE', 
...
WHERE table_database = 'SNOWFLAKE_EVAL'
